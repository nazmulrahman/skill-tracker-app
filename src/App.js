...
const AdminDashboard = () => {
    const { employees, addEmployee, deleteEmployee, setCurrentPage, setCurrentEmployeeId } = useSkillTracker();
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (employeeName && employeeRole && !isNaN(parseFloat(employeeTimeAtRole)) && parseFloat(employeeTimeAtRole) >= 0) {
            addEmployee(employeeName, employeeRole, parseFloat(employeeTimeAtRole));
            setEmployeeName('');
            setEmployeeRole('');
            setEmployeeTimeAtRole('');
        } else {
            alert('Please fill in all employee details correctly.');
        }
    };
        ...
        <div className="container mx-auto p-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow mb-8">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label htmlFor="employee-name" className="block text-sm font-medium text-gray-700">Employee Name</label>
                        <input
                            type="text"
                            id="employee-name"
                    ...
                    <div>
                        <label htmlFor="employee-role" className="block text-sm font-medium text-gray-700">Role</label>
                        <select
                            id="employee-role"
                            className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"
                            value={employeeRole}
                        ...
                        >
                            ...
                            {/* Adding "Assistant Engineer" to the dropdown as requested */}
                    ...
                    <div>
                        <label htmlFor="employee-time-at-role" className="block text-sm font-medium text-gray-700">Time at Role (Years)</label>
                        <input
                            type="number"
                            id="employee-time-at-role"
                            className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"
                            step="0.1"
            ...
            <div className="bg-white p-6 rounded-lg shadow">
                <h2 className="text-2xl font-semibold text-gray-700 mb-4">Manage Employees</h2>
                <div className="space-y-4">
                    {employees.length === 0 ? (
                        <p className="text-gray-500">No employees added yet. Use the form above to add one.</p>
                    ) : (
                        employees.map(employee => (
                            <div key={employee.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg shadow-sm">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">Time at Role: {employee.timeAtRole.toFixed(1)} years | Total Years in Company: {employee.totalYearsInCompany.toFixed(1)} years</p>
                                    <p className="text-sm text-gray-600">Calculated Score: <span className="font-bold">{employee.calculatedData.totalScore}</span> | Calculated Level: <span className="font-bold">{employee.calculatedData.levelData.level}</span></p>
                                </div>
                                <div className="space-x-2">
                                    ...
                                    <button
                                        ...
                                        className="delete-employee-btn bg-red-500 text-white px-3 py-1 rounded-md hover:bg-red-600"
                                    >
                                        Delete
                                    </button>
                                </div>
                    ...
                    {localSkills.map(skill => (
                        <div key={skill.name} className="flex flex-col">
                            ...
                            <input
                                ...
                                onChange={(e) => handleLast6MonthsScoreChange(skill.name, parseInt(e.target.value))}
...
const EmployeeDetail = ({ employee, isAdminContext }) => {
    ...
    const handleLevelChange = (e) => {
        ...
        if (newLevelIndex < currentLevelIndex) {
             if (window.confirm(`Are you sure you want to promote ${localEmployee.name} to ${newLevel}? This will reset their 'Time at Role' and add it to 'Total Years in Company'.`)) {
                setLocalEmployee(prevEmployee => ({
                    ...prevEmployee,
                ...
                alert(`${localEmployee.name} promoted to ${newLevel}!`);
            } else {
    ...
    return (
        <div className="container mx-auto p-6">
            ...
            <h1 className="text-3xl font-bold text-gray-800 mb-4">
                {localEmployee.name}'s Skills Assessment
            </h1>
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {employees.map(employee => (
                                    <tr key={employee.id} className="hover:bg-gray-50">
                                        <td className="py-3 px-4 border-b border-gray-200 text-gray-800">{employee.name}</td>
                                        <td className="py-3 px-4 border-b border-gray-200 text-gray-800">{employee.role}</td>
                                        <td className="py-3 px-4 border-b border-gray-200 text-gray-800">{employee.currentLevel}</td>
                                        ...
                                        <td className="py-3 px-4 border-b border-gray-200 text-gray-800">{employee.calculatedData.totalScore}</td>
                                        <td className="py-3 px-4 border-b border-gray-200 text-gray-800">{employee.calculatedData.levelData.level}</td>
                                        <td className="py-3 px-4 border-b border-gray-200 text-gray-800">{employee.calculatedData.bestFitRole}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
